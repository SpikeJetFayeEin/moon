...
def test_calculates_period_drawdowns_by_calendar_boundaries():
    nav_points = [
        {"date": date(2023, 12, 29), "nav": 1.00},
        {"date": date(2024, 1, 2), "nav": 1.20},
        {"date": date(2024, 7, 1), "nav": 1.00},
        {"date": date(2024, 9, 2), "nav": 0.80},
        {"date": date(2024, 12, 31), "nav": 1.10},
    ]

    metrics = calculate_fund_metrics(nav_points)

    assert metrics.period_drawdowns["6m"] == pytest.approx(-0.20)
    assert metrics.period_drawdowns["1y"] == pytest.approx((0.80 / 1.20) - 1)
    assert metrics.period_drawdowns["3y"] is None
    assert metrics.period_drawdowns["since_inception"] == pytest.approx((0.80 / 1.20) - 1)
